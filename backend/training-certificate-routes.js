...
async function issueForEnrollment(enrollmentId){
  ...
  const cert=await Certificate.create({certificateId,traineeId:t.traineeId,enrollmentId,courseId:e.courseId,courseNameEn:e.courseNameEn,courseNameAr:e.courseNameAr,recipientNameEn:nameEn,recipientNameAr:nameAr,email:t.email,verificationUrl});
...
install('get','/api/training-certificates/:certificateId',async(req,res)=>{try{const c=await Certificate.findOne({certificateId:clean(req.params.certificateId,100)}).lean();if(!c)return res.status(404).json({message:'Certificate not found'});res.json(c)}catch(err){res.status(500).json({message:'Unable to load certificate'})}});
