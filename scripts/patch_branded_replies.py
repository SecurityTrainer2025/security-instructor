...
if "require('nodemailer')" not in text:
    text = text.replace("const crypto=require('crypto');", "const crypto=require('crypto');\nconst nodemailer=require('nodemailer');", 1)

anchor = "function visitorView(v){return {visitorId:v.visitorId,name:v.name,email:v.email,linkedin:v.linkedin,egyptPhone:v.egyptPhone,outlookEmail:v.outlookEmail,consent:v.consent,registeredAt:v.registeredAt,challengeStartedAt:v.challengeStartedAt,challengeCompletedAt:v.challengeCompletedAt,score:v.score}}"
if 'function buildContactReplyHtml(' not in text:
    insert = r'''
...
admin = Path('frontend/contact-admin.js')
admin.write_text(r'''(function(){
  function init(){
...
print('Branded reply patch prepared')
