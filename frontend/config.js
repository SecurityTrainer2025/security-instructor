...
window.addEventListener('DOMContentLoaded',function(){
  ...
  if(nav && !nav.querySelector('a[href="challenge.html"]')){
    ...
    link.setAttribute('aria-label','Security Knowledge Challenge / اختبر معلوماتك الأمنية');
  ...
  // Add the dedicated course assessment access to the Traffic Management card.
  var trafficBtn=document.querySelector('[data-course="traffic-management-vehicle-control"]');
  if(trafficBtn && !document.querySelector('[data-course-assessment="traffic-management-vehicle-control"]')){
    var assessment=document.createElement('a');
    assessment.href='course-assessment.html?course=traffic-management-vehicle-control';
    assessment.setAttribute('data-course-assessment','traffic-management-vehicle-control');
    assessment.innerHTML='Course Assessment / تقييم الدورة';
    assessment.style.cssText='display:inline-block;margin:10px 8px 0 0;padding:11px 16px;border:1px solid #c8a96b;color:#c8a96b;background:transparent;text-decoration:none;font-weight:800;';
    trafficBtn.insertAdjacentElement('afterend',assessment);
  }
